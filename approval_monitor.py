...
def main():
    ...
    try:
        ...
        endpoint=service_settings.get('targetendpoint')
    ...
    if is_available :
        ...
        lineas_actualizadas=Request.Integracion(PGSS_LINEASCREDITO).add()
        try:
            results=db.get(lineas_actualizadas)
        except Exception:
        ...
        if(results.status_code==0):
            ...
            if(results.rowcount>0):
                ...
                json_requests=results.to_json(key='idLineaCredito')
                response_list=[]
                if not Utils.is_reachable(endpoint): 
                    ...
                    for json in json_requests:
                        ...
                        response={}
                        #print (json)
                        LineaCreditoID=json['key']
                        json_string=json['string']
                    
             
                        try :
                            logger.debug(f'Procesando InstrumentoID { LineaCreditoID }')
                ...
                if save_response:
                    ...
                    for response in response_list:
                        ...
                        if response['status_code']=='200':
                            is_success=True
                        try:
                            r=save_request(response,end_point=save_req_endpoint ,success=is_success)
                        except Exception as e:
                            error=error+1
                            if(error==1):
